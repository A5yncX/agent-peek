...
const MESSAGES = {
  en: {
    ...
    looking: 'Reading progress', cancelHint: '/peek cancel to stop',
    optionsTitle: 'Agent Peek · Options', confirmationOption: value => `Summary confirmation  ${value ? 'On · ask before sending' : 'Off · send without asking'}`,
    displayOption: value => `Result display  ${value === 'window' ? 'Window · temporary' : 'Conversation · saved'}`,
    closeOptions: 'Close',
  ...
  zh: {
    ...
    looking: '正在读取进度', cancelHint: '/peek cancel 取消',
    optionsTitle: 'Agent Peek · 选项', confirmationOption: value => `摘要前确认  ${value ? '开启 · 发送前询问' : '关闭 · 直接发送'}`,
    displayOption: value => `结果展示    ${value === 'window' ? '窗口 · 临时查看' : '对话 · 保留结果'}`,
    closeOptions: '关闭',
